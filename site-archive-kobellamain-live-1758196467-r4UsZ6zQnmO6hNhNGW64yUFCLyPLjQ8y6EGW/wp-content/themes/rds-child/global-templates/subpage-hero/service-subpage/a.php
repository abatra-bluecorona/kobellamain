                    ...
                    if ($loop->have_posts()) {
                        while ($loop->have_posts()): $loop->the_post();
                            ...
                            if (strtotime($date) >= strtotime(current_time("m/d/Y")) || $noexpiry == 1) {
                                ...
                                <a data-bs-toggle="modal" data-bs-target="#service_subpage_request_coupon_form" onclick="couponBannerButtonClick(this);" class="btn btn-primary mw-226 mt-2">Request Service <i class="icon-chevron-right text_18 line_height_18 ms-2"></i></a>
                                <?php
                            }
                        endwhile;
                        wp_reset_postdata();
...
<div class="modal fade request_form px-lg-0 px-0 pt-5 pt-md-0" id="service_subpage_request_coupon_form" tabindex="-1" role="dialog" data-bs-backdrop="false" data-bs-keyboard="false" aria-labelledby="requestcoupon_Label" aria-hidden="true">
        ...
        <div class="modal-content border-0 rounded-0 text-center">
            ...
            <div class="modal-body p-lg-4 p-2 w-100 my-auto mx-auto coupons">
                <div class="border-dashed-7 pt-lg-4 pb-lg-4 py-4 footer_form_A ui_kit_footer_form elementor-popupform">
                    <?php
                    // Check if popup_form_heading and popup_form_subheading are not empty before displaying
                    if (!empty($args['page_templates']['subpage']['sidebar']['promotion']['popup_form_heading'])) {
                        ?>
                        <h3 class="px-lg-0 px-4"><?php echo esc_html($args['page_templates']['subpage']['sidebar']['promotion']['popup_form_heading']); ?></h3>
                        <?php
                    }

                    if (!empty($args['page_templates']['subpage']['sidebar']['promotion']['popup_form_subheading'])) {
                        ?>
                        <div class="my-md-0 mt-lg-4 mt-3 w-lg-260 mx-auto text-start text-lg-center d-flex align-items-center justify-content-center pb-4 px-lg-0 px-4">
                            ...
                            <span class="font_alt_1 text_bold text_16 line_height_25 sm_text_16 sm_line_height_30 color_primary"><?php echo esc_html($args['page_templates']['subpage']['sidebar']['promotion']['popup_form_subheading']); ?></span>
                        </div>
                        <?php
                    }

                    // Check if popup_gravity_form_id is not empty before displaying the form
                    if (!empty($args['page_templates']['subpage']['sidebar']['promotion']['popup_gravity_form_id'])) {
